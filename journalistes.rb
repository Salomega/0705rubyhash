...
def nombre_journaliste(tableau)
	puts "   => Il y a #{tableau.length} journalistes dans cet array" 
end
nombre_journaliste(handles)

puts " "
puts "-------- :-) ----------"
puts "2) Combien d'handle contiennent un numéro ?"

def contient_numero(mots)
	i = 0
	mots.each do |mot|
		if #mot.count("0-9") > 0
			mot =~ /\d/
...
contient_numero(handles)

puts " "
puts "-------- :-) ----------"
puts "3.a) Combien d'handle contiennent les 4 lettres du prénom Aude"
puts "  dans le désordre (sans faire attention à la casse) ? "

def find_chars(subject, characters)
  characters.chars.all? { |char| subject.include?(char) }
end

def contient_aude_a(mots)
	i = 0
	mots.each do |mot|
		mot_min = mot.downcase
		if find_chars(mot_min , "aude")
			i += 1
		end
	end
	print "   => "    
...
contient_aude_a(handles)

puts " "
puts "-------- :-) ----------"
puts "3.b) Combien d'handle contiennent les 4 lettres du prénom Aude"
puts "  dans l'ordre (sans faire attention à la casse) ? "

def contient_aude_b(mots)
	i = 0
	mots.each do |mot|
		if mot =~ /Aude/i
			i += 1
...
puts " "
puts "-------- :-) ----------"
puts "5) Combien de mots contienent une majuscule ? "

def capitale_bis (mots)
	i = 0
	mots.each do |mot|
		if mot.downcase != mot
			i += 1
...
def ordre_alphabetique(mots)
	p mots.sort_by(&:downcase)
end

ordre_alphabetique(handles)
